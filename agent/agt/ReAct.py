...
from agent.agt.Action import Action
from agent.utils.CallbackHandlers import ColoredPrintHandler
from agent.utils.PrintUtils import *
...
class ReActAgent:
    def __init__(
            ...
            max_thought_steps: Optional[int] = 10,  # 最多执行多少步
    ):
        self.llm = llm
        self.tools = tools
        ...
        self.__init_prompt_templates()  # 初始化prompt模板
        self.__init_chains()  # 初始化chain

        self.verbose_handler = ColoredPrintHandler(color=THOUGHT_COLOR)

    def __init_prompt_templates(self):
        # 从文件加载Prompt模板，并转换成LangChain的模板
        with open(self.main_prompt_file, 'r', encoding='utf-8') as f:
            self.prompt = ChatPromptTemplate.from_messages(
                ...
                tools=render_text_description(self.tools),  # render_text_description 函数的作用是将我们定义的工具转换成文字描述
                tool_names=','.join([tool.name for tool in self.tools]),
                format_instructions=self.output_parser.get_format_instructions(),  # 输出格式
            )

    def __init_chains(self):
        # 主流程的chain：输入prompt模板 -> 调用大模型 -> 输出
