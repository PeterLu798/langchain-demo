...
2、简单介绍该智能体   
* 本智能体提供三个文件，在 agent/data 目录下，一个是销售记录，一个是供应商名录，一个是供应商资格要求。   
* 提供的工具集在 agent/tools 下  
* 智能体整体功能：根据提供的文件数据以及工具集，来让大模型回答用户相关问题，比如统计销售额、发送邮件等等
* 主prompt在 agent/prompts 下
...
* 整体结构分为三块：
    ...
    + 中间步骤原则 
        - 必须要告诉大模型结束标志
        - 确保大模型调用的指令或使用的工具在给定的工具列表中
        - 还有一些其他原则，具体看 “你必须遵循以下约束来完成任务” 一节中
    + 输出 
        - 首先考虑是否输出思考过程
        - 告诉大模型最终输出结构
* 思考过程中的一些关键点
    ...
    + 计划校验：校验之前的计划是否有纰漏，是否有可直接带入的数据等
    + 计划改进：如果某一步卡住了，或者报bug了，告诉大模型接下来怎么办
